void ListMoves(ChessBoard current_board, vector<ChessMove> move_list) {
	for (int i = 0; i < move_list.size(); i++) {
		...
		cout << i+1 << ": " << piece << IntToFile(start_file) << 8-start_rank << " -> " << piece << IntToFile(end_file) << 8-end_rank << endl;
	}
}