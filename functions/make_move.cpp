ChessBoard MakeMove(ChessBoard current_board, ChessMove given_move) {
	...
	// Check if board is in check for next color (after move has been made)
	if (CheckCheck(current_board) == 1)
	{
		current_board.check = current_board.color;
	} else
	{
		current_board.check = 2;
	}
