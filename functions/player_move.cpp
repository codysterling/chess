ChessBoard PlayerMove(ChessBoard current_board) {
	// This is the main player move function
	...
	while (cin)
	{
		...
		if (parsed_move.start[0] == 8 || parsed_move.start[1] == 8 || parsed_move.end[0] == 8 || parsed_move.end[1] == 8)
		{ // Parser returns bad move
			cout << "You have entered an illegal or unclear move.  Please enter a new one." << endl;
			cin.clear();
			cin.ignore(numeric_limits<streamsize>::max(), '\n');
		}
		else
		{ // Otherwise good move, break to do move
			break;
		}
	}

	current_board = MakeMove(current_board, parsed_move);
